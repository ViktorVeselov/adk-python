...
- Flat Key-Value JSON: Simple id/parentId/name/definition structure.
- JSON-LD with SKOS: Semantic web vocabularies (prefLabel, altLabel, definition, broader).
"""
...
class TaxonomyRegistry(BaseModel):
  ...
  def from_json_ld(cls, data: list[dict]) -> TaxonomyRegistry:
        ...
        [
            {
                ...
                "definition": {
                    "@value": "A branch of AI focused on building systems that learn from data.",
                    "@language": "en"
                },
                "broader": "https://example.com/ai"
            }
        ]
    """
    terms = {}
    for item in data:
      ...
      # PrefLabel may be a dict with @value or a plain string
      ...
      # AltLabels list of dicts with @value
