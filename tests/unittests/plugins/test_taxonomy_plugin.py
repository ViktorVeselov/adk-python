...
"""Unit tests for the Pluggable Policy & Taxonomy Security Engine."""

from unittest import mock
import pytest

from google.adk.plugins.taxonomy import DefaultSkillPolicy
from google.adk.plugins.taxonomy import SkillPolicy
from google.adk.plugins.taxonomy import TaxonomyPipeline
from google.adk.plugins.taxonomy import TaxonomyPlugin
from google.adk.plugins.taxonomy import TaxonomyRegistry
from google.adk.plugins.taxonomy import TaxonomyResolver
from google.adk.plugins.taxonomy import TaxonomyTerm
...
async def test_taxonomy_pipeline():
  ...
  # Pipeline with multiple resolvers
...
def test_default_skill_policy():
  ...
  # Check validation against active taxonomies
  context = mock.MagicMock()
  ...
  # Skill with no taxonomy-binds is allowed by default
  ...
  # Original instructions should be preserved unmodified
...
async def test_taxonomy_plugin_list_skills():
  ...
  class RestrictedPolicy(SkillPolicy):
    ...
    def is_skill_allowed(self, skill: Skill, context, active_taxonomies: list[str]) -> bool:
      return "eng" in skill.frontmatter.taxonomy_binds
  ...
  context = mock.MagicMock()
  # Mock active taxonomies in tool context state
  context.state = {"_active_taxonomies": ["eng"]}

  # Mock the tool object for list_skills
  mock_tool = mock.MagicMock()
  mock_tool.name = "list_skills"
  mock_tool._toolset._list_skills.return_value = list(skills.values())

  # Set up mocks for skill filtering within plugin
  with mock.patch("google.adk.plugins.taxonomy.taxonomy_plugin.prompt.format_skills_as_xml") as mock_format:
    mock_format.return_value = "<skills><skill name=\"skill-1\"/></skills>"

    # Trigger list_skills interceptor via before_tool_callback
    ...
    # Must return dictionary wrapped XML per contract
